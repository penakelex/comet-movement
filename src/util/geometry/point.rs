...
pub fn translate_point<T: Num>(
    ...
    let delta_x =
        previous_center_point.x - new_center_point.x;
    let delta_y =
        previous_center_point.y - new_center_point.y;

    Point::new(
        point_in_previous_system.x + delta_x,
        point_in_previous_system.y + delta_y,
    )
}
